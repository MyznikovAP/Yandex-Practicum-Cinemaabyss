...
А именно:
  ...
  - Добавил описание компонента Movies
    - Файлы хранятся в S3 (фильмы)
    - Elastic хранит и отдает инфорацию по фильмам (мета-инфа)
    - Redis кэширует запросы для выдачи контента (фильмы)
  - Добавил компоненты по обработке Auth
```markdown
[ссылка на файл](schemas/_index.puml)
```
